...
(function () {
    ...
    function checkElementII(callback) {
        ...
        let checkPlaylist = window.data === undefined || window.data.length == 0;

        let checkInsertAnchorI = $('#songs > h4.dq').length == 0;
        

        let test = checkInsertAnchorI || checkPlaylist;
        if (test 
            && count < limit) {
    ...
    function insertButton() {
        ...
        $("#songs > h4.dq > a").after('<a id="c4r-exportPlaylist" class="pc_temp_btn_s02 " title="导出" hidefocus="true"><span><i class="t2"></i>导出</span></a>');
    ...
    function savePlaylist() {
        ...
        window.data.forEach(element => {
            str_store = str_store + element.audio_name + " - " + element.author_name + "\r\n";
        });
